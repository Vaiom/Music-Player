...
class Ui_UnidentifiedPlayer(object):
    def setupUi(self, UnidentifiedPlayer):
        ...
        self.song_list = []
        self.title, self.artist, self.duration, self.file, self.ftype, self.fsize, self.bitrate, self.directory, self.rawsecs = get_song_data(self.fname)
        self.nulltime = 0
        ...
        sizePolicy.setHeightForWidth(UnidentifiedPlayer.sizePolicy().hasHeightForWidth())
    ...
    def retranslateUi(self, UnidentifiedPlayer):
        ...
        UnidentifiedPlayer.setWindowTitle(_translate("UnidentifiedPlayer", "Unidentified Player"))
        ...
        self.shufflebutton.setText(_translate("UnidentifiedPlayer", "Shuffle: Off"))
        self.repeatbutton.setText(_translate("UnidentifiedPlayer", "Repeat: Off"))
        self.stopsongbutton.setText(_translate("UnidentifiedPlayer", "■"))
        self.loadsongsbutton.setText(_translate("UnidentifiedPlayer", "Add Song(s)"))
        self.removeselectionbutton.setText(_translate("UnidentifiedPlayer", "Remove Selection"))
        self.songtimelabel.setText(_translate("UnidentifiedPlayer", "00:00"))
        self.volumevaluelabel.setText(_translate("UnidentifiedPlayer", str(self.volume_level)))
        self.artistlabel.setText(_translate("UnidentifiedPlayer", "Artist:"))
        self.titlelabel.setText(_translate("UnidentifiedPlayer", "Title:"))
        self.durationlabel.setText(_translate("UnidentifiedPlayer", "Duration:"))
        ...
        self.infolabel.setText(_translate("UnidentifiedPlayer", "(Note: Currently, only mp3s and flacs are supported!)"))
        self.updatelabels()
    ...
    def add_songs(self):
        try:
            self.fname = QFileDialog.getOpenFileNames(None, 'Add Song(s)', 'c:\\Users\\' + getpass.getuser() + '\\Music', "Audio Files (*.mp3 *.flac)")
    ...
    def loader(self):
        try:
            ...
            self.title, self.artist, self.duration, self.file, self.ftype, self.fsize, self.bitrate, self.directory, self.rawsecs = get_song_data(self.songd)
            self.updatelabels()
            self.playing_state = False
    ...
    def thread_end(self):
        self.t2=Thread(target=self.song_end)
        self.t2.start()

    def play_pause(self):
        if self.ready_to_play == False:
            self.loader()
        if self.ready_to_play == True and self.playing_state == False:
            mixer.music.unpause()
            self.slider_pressed = False
            self.playing_state = True
        ...
        elif self.ready_to_play == True and self.playing_state == True:
            mixer.music.pause()
            self.playing_state = False
            self.playpausebutton.setText("►")
    ...
    def song_end(self):
        ...
        if self.shuffle == "on":
            self.song_list = [self.list_widget.item(i).text() for i in range(self.list_widget.count())]
    ...
    def stop(self):
        mixer.music.stop()
        mixer.music.unload()
        self.title, self.artist, self.duration, self.file, self.ftype, self.fsize, self.bitrate, self.directory, self.rawsecs = get_song_data("")
        self.updatelabels()
        self.fname = None
        self.playing_state = False
        self.ready_to_play = False
